...
**Goal:** Create a new Streamlit dashboard (`dashboard_v2.py`) with dark editorial theme, three pages (Session Explorer, Version Comparison, Failure Analysis), chat-style conversation view with inline annotations.

**Architecture:** Single-file Streamlit app with custom CSS theme injected via markdown. Uses existing `TraceStore` for data access. No new dependencies.

**Tech Stack:** Streamlit, Python 3.10+, existing llm_eval package (models, store)

---

## Task 0: Create file scaffold and theme CSS

**Files:**
- Create: `llm_eval/dashboard_v2.py`
...
python -c "import llm_eval.dashboard_v2"
```

Expected: No errors (just imports successfully)

**Step 3: Commit scaffold**

```bash
git add llm_eval/dashboard_v2.py
git commit -m "feat: add dashboard_v2.py scaffold with theme CSS"
```

---

## Task 1: Implement Page 1 - Session Explorer

**Files:**
- Modify: `llm_eval/dashboard_v2.py`

**Step 1: Add render_message helper function**

Add after `truncate_text` function:
...
**Step 2: Add render_trace function**

Add after `render_message`:
...
**Step 3: Implement page_sessions function**

Replace the placeholder `page_sessions` function:
...
**Step 4: Verify page runs**

Run:
```bash
cd /Users/L122739/Library/CloudStorage/OneDrive-EliLillyandCompany/Desktop/evals/phase_3_evals
LLM_EVAL_DB_PATH=test_traces.db streamlit run llm_eval/dashboard_v2.py
```

Expected: Dashboard opens, Sessions page shows session list and traces

**Step 5: Commit**

```bash
git add llm_eval/dashboard_v2.py
git commit -m "feat: implement Session Explorer page with chat-style view"
```

---

## Task 2: Implement Page 2 - Version Comparison

**Files:**
- Modify: `llm_eval/dashboard_v2.py`

**Step 1: Replace page_versions function**
...
Expected: Side-by-side comparison of prompt versions with stats

**Step 3: Commit**

```bash
git add llm_eval/dashboard_v2.py
git commit -m "feat: implement Version Comparison page"
```

---

## Task 3: Implement Page 3 - Failure Analysis

**Files:**
- Modify: `llm_eval/dashboard_v2.py`

**Step 1: Add stub LLM analysis function**
...
**Step 2: Replace page_failures function**
...
Expected: List of failures with checkboxes, analyze button, placeholder results

**Step 4: Commit**

```bash
git add llm_eval/dashboard_v2.py
git commit -m "feat: implement Failure Analysis page with LLM stub"
```
...
```bash
cd /Users/L122739/Library/CloudStorage/OneDrive-EliLillyandCompany/Desktop/evals/phase_3_evals
python -c "import llm_eval.dashboard_v2; print('Import OK')"
```

Expected: "Import OK"

**Step 2: Test with existing test database**
...
Verify:
- [ ] Sessions page loads with session list
- [ ] Clicking session shows traces
- [ ] Chat-style message display works
- [ ] Annotation save works
- [ ] Versions page shows side-by-side comparison
- [ ] Failures page shows failure list and analysis

**Step 3: Final commit**

```bash
git add llm_eval/dashboard_v2.py
git commit -m "feat: complete dashboard_v2.py implementation

New Streamlit dashboard with:
- Dark editorial theme
- Page 1: Session Explorer with chat-style view and inline annotations
- Page 2: Version Comparison with side-by-side stats
- Page 3: Failure Analysis with LLM stub for categorization

Run with: streamlit run llm_eval/dashboard_v2.py"
```
...
- [ ] `dashboard_v2.py` creates without errors
- [ ] Dark editorial theme applied (dark background, blue/green accents)
- [ ] Page 1 shows sessions, traces in chat format, inline annotations
- [ ] Page 2 shows side-by-side version comparison with stats
- [ ] Page 3 shows failure selection and placeholder analysis
- [ ] No new dependencies added (uses existing streamlit)
- [ ] Existing dashboard.py unchanged
